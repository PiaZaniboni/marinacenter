    ...
    */
    public static function valType($string)
    {
        if(!is_array($string) && !is_object($string))
        {
            ...
            if(is_numeric($string) && (float)$string == $string)
                return (float)$string;

            if(is_string($string) && in_array(strtolower($string), array('true','false'), true) !== false)
                return (strtolower($string) === 'true') ? true : false;	// can't use boolval since we have to support php 5.2
